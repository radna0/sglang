...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Dense MLA + LoRA healing for GPT-OSS CARE MLA checkpoints.")
...
def main() -> None:
    ...
    try:
        ...
        raise RuntimeError("peft is required for H2 dense-heal LoRA training.") from exc

    output_dir = Path(args.output_dir).resolve()
    output_dir.mkdir(parents=True, exist_ok=True)
    manifest_path = output_dir / "dense_heal_lora_manifest.json"
    log_path = output_dir / "dense_heal_lora_log.jsonl"
    ...
    all_trainable = [param for _, param in student_model.named_parameters() if param.requires_grad]
    trainable_params = gather_unique_trainable_params(list(mla_params) + all_trainable)
    if not trainable_params:
        raise RuntimeError("No trainable H2 parameters were selected.")
    ...
    manifest = {
        "phase": "dense_heal_mla_plus_lora_h2",
        ...
        "trainable_subset": ["q_proj", "kv_a_proj_with_mqa", "kv_b_proj"] + ["LoRA:q_proj", "LoRA:o_proj", "LoRA:gate_proj", "LoRA:up_proj", "LoRA:down_proj"],
        "trainable_param_count": int(sum(p.numel() for p in trainable_params)),
        "healing_stage": "dense_mla_plus_lora_h2",
    }
    ...
    print(f"[done] wrote dense-healed LoRA checkpoint to {final_dir}", flush=True)
