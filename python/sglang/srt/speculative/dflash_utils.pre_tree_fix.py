...
def build_dflash_tree_candidates_from_per_step_topk(
    *,
    ...
    candidate_scores_buf: Optional[torch.Tensor] = None,
    candidate_tokens_buf: Optional[torch.Tensor] = None,
    parent_list_buf: Optional[torch.Tensor] = None,
    top_scores_index_buf: Optional[torch.Tensor] = None,
) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    """Build DFLASH_TREE candidates from per-step top-k outputs.

    This preserves the reference tree-building semantics used by the legacy
    draft worker path and the unit tests. The optional output buffers let the
    CUDA-graph path reuse allocations without changing the logical result.
    """
    from sglang.srt.speculative.eagle_utils import organize_draft_results
    from sglang.srt.speculative.spec_utils import select_top_k_tokens

    if topk_p.ndim != 3 or topk_index.ndim != 3:
        raise ValueError(
            "topk_p and topk_index must be rank-3 tensors of shape "
            "[bs, step_count, topk]."
        )
    if tuple(topk_p.shape) != tuple(topk_index.shape):
        raise ValueError(
            "topk_p and topk_index must have identical shapes, got "
            f"{tuple(topk_p.shape)} vs {tuple(topk_index.shape)}."
        )

    bs, step_count, topk = [int(x) for x in topk_p.shape]
    if bs <= 0 or step_count <= 0 or topk <= 0:
        raise ValueError(
            "topk_p/topk_index must have positive batch, step, and topk sizes, got "
            f"{tuple(topk_p.shape)}."
        )
    ...
    parent_list, top_scores_index, draft_tokens = organize_draft_results(
        ...
        token_list,
        parents_list,
        num_draft_token=int(num_verify_tokens),
    )

    if candidate_scores_buf is not None:
        flat_scores = torch.cat(
            [scores.reshape(bs, -1) for scores in score_list],
            dim=1,
        )
        if candidate_scores_buf.shape[0] < bs or candidate_scores_buf.shape[1] < flat_scores.shape[1]:
            raise ValueError(
                "candidate_scores_buf is too small for the generated tree scores: "
                f"buf={tuple(candidate_scores_buf.shape)} needed={(bs, int(flat_scores.shape[1]))}."
            )
        candidate_scores_buf[:bs, : flat_scores.shape[1]].copy_(flat_scores)

    if candidate_tokens_buf is not None:
        flat_tokens = torch.cat(
            [tokens.reshape(bs, -1) for tokens in token_list],
            dim=1,
        )
        if candidate_tokens_buf.shape[0] < bs or candidate_tokens_buf.shape[1] < flat_tokens.shape[1]:
            raise ValueError(
                "candidate_tokens_buf is too small for the generated tree tokens: "
                f"buf={tuple(candidate_tokens_buf.shape)} needed={(bs, int(flat_tokens.shape[1]))}."
            )
        candidate_tokens_buf[:bs, : flat_tokens.shape[1]].copy_(flat_tokens)

    if parent_list_buf is not None:
        if parent_list_buf.shape[0] < bs or parent_list_buf.shape[1] < parent_list.shape[1]:
            raise ValueError(
                "parent_list_buf is too small for the generated parent list: "
                f"buf={tuple(parent_list_buf.shape)} needed={(bs, int(parent_list.shape[1]))}."
            )
        parent_list_buf[:bs, : parent_list.shape[1]].copy_(parent_list)
        parent_list = parent_list_buf[:bs, : parent_list.shape[1]]

    if top_scores_index_buf is not None:
        if top_scores_index_buf.shape[0] < bs or top_scores_index_buf.shape[1] < top_scores_index.shape[1]:
            raise ValueError(
                "top_scores_index_buf is too small for the generated index list: "
                f"buf={tuple(top_scores_index_buf.shape)} needed={(bs, int(top_scores_index.shape[1]))}."
            )
        top_scores_index_buf[:bs, : top_scores_index.shape[1]].copy_(top_scores_index)
        top_scores_index = top_scores_index_buf[:bs, : top_scores_index.shape[1]]
