...
class DFlashWorker:
    ...
    def __init__(
        ...
        # The DFLASH draft model consumes target embeddings and does not have its own
        # standalone decode graph path. Keep draft-worker graphs off; the target worker
        # owns the verify path and graph capture for linear DFLASH.
    ...
    def _prepare_for_speculative_decoding(
        ...
        global _DFLASH_FIRST_BLOCK_LOGGED
        if (
            not _DFLASH_FIRST_BLOCK_LOGGED
            and self.tp_rank == 0
            ...
            try:
                ...
                if tp_size == 1 and int(self.block_size) > 1:
                    hs_rows = draft_hidden[0, 1 : min(self.block_size, 4), :]
                    hs_rows = hs_rows if hs_rows.dtype == weight.dtype else hs_rows.to(weight.dtype)
                    num_org = int(shard.num_org_elements)
                    num_org_padded = int(shard.num_org_elements_padded)
                    num_added = int(shard.num_added_elements)
                    org_vocab_start = int(shard.org_vocab_start_index)
                    ...
                    id_parts = []
                    if num_org > 0:
                        base_logits = torch.matmul(hs_rows, weight[:num_org].T)
                        logits_parts.append(base_logits)
                        id_parts.append(
                            torch.arange(
                                num_org,
                                device=hs_rows.device,
                                dtype=torch.long,
                            )
                            + org_vocab_start
                        )
                    if num_added > 0:
                        ...
                        added_logits = torch.matmul(
                            hs_rows, weight[added_slice_start:added_slice_end].T
                        )
                        logits_parts.append(added_logits)
                        id_parts.append(
                            torch.arange(
                                num_added,
                                device=hs_rows.device,
                    ...
                    if logits_parts:
                        full_logits = torch.cat(logits_parts, dim=-1)
                        full_ids = torch.cat(id_parts, dim=0)
                        k = min(8, int(full_logits.shape[-1]))
                        top_vals, top_idx = torch.topk(full_logits, k=k, dim=-1)
                        top_ids = full_ids.index_select(0, top_idx.reshape(-1)).view_as(top_idx)
                        trace_payload["draft_topk_ids"] = (
                            top_ids.detach().to("cpu", non_blocking=False).tolist()
                        )
                        trace_payload["draft_topk_vals"] = (
                            top_vals.detach().to("cpu", non_blocking=False).tolist()
                        )
            except Exception as exc:
                trace_payload = {"lm_head_trace_error": repr(exc)}
            logger.info("DFLASH first draft lm_head trace: %s", trace_payload)
        ...
        if (
            ...
            logger.info(
                "DFLASH first draft block: prefix_len=%s verified_id=%s block_ids=%s draft_tokens=%s positions=%s",
                int(draft_prefix_lens[0].item()),
                int(draft_input.verified_id[0].item()),
                block_ids[0].detach().to("cpu", non_blocking=False).tolist(),
                draft_tokens[0].detach().to("cpu", non_blocking=False).tolist(),
                positions_2d[0].detach().to("cpu", non_blocking=False).tolist(),
            )
            _DFLASH_FIRST_BLOCK_LOGGED = True
