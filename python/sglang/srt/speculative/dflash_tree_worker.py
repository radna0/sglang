...
class DFlashTreeWorker:
    ...
    def __init__(
        ...
        **_unused_kwargs,
        ...
        if draft_backend is None:
            draft_backend = "fa3"
        elif draft_backend != "fa3":
            raise ValueError(
                "DFLASH_TREE on the clean baseline requires draft attention backend 'fa3'. "
        ...
        # The draft model consumes target embeddings explicitly. Until the clean tree
        # baseline wires a dedicated draft graph path, keep generic draft-worker graph
        # capture disabled and let the target worker own CUDA-graph capture.
        ...
        self._draft_model_requires_input_embeds = bool(
            getattr(self.draft_model, "requires_input_embeds", False)
        )
        if hasattr(self.draft_model, "set_embed") and not bool(
            getattr(self.draft_model, "requires_input_embeds", False)
        ):
        ...
        if self.tp_rank == 0:
            ...
            if self._draft_model_requires_input_embeds:
                logger.info(
                    "DFLASH_TREE draft model requires explicit target input_embeds; enabling that path automatically."
                )
        ...
        if is_cuda() and not bool(self.server_args.disable_cuda_graph) and self.tp_rank == 0:
            logger.info(
                "DFLASH_TREE draft cuda graph runner is deferred on the clean baseline; using eager draft replay."
            )
    ...
    def _draft_tree_build_candidates_legacy(
        ...
        *,
        ...
        if draft_hidden is None:
            raise RuntimeError("DFLASH_TREE draft model returned no hidden states.")
    ...
    def _prepare_for_speculative_decoding(
        ...
        *,
        ...
        embed_module = None
        if self._force_explicit_input_embeds or self._draft_model_requires_input_embeds:
