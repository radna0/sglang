...
if (os.environ.get("SGLANG_FLEX2_IMPORT_DEBUG") or "").strip() in _TRUE_ENVS:
    try:
        print("[flex2] module import ok (BlockMask imported)", flush=True)
    except Exception:
        pass
...
class TorchFlexAttnBackendV2(TorchFlexAttnBackend):
    ...
    def __init__(self, model_runner: "ModelRunner", *, kernel_options: dict | None = None):
        # FlexAttention relies on TorchInductor (and typically Triton/Cutlass) for kernel generation.
        # Ensure at least one GEMM backend is enabled for autotuning, otherwise inductor can raise
        # NoValidChoicesError during flex kernel compilation on some builds.
        #
        # IMPORTANT: prefer env vars over importing torch._inductor here; importing it can be slow
        # and has occasionally deadlocked under containerized multi-process launchers.
        ...
        # `force_gather` disables the V2 paged global-KV fastpath and forces the
        # gather-based FlexAttention path (V1 behavior). This is the safety mode for
        # production-like inference because some PyTorch builds will materialize the
        # broadcasted (B,H,KV,D) K/V tensors under CUDA-graph capture, causing 64+GiB OOM.
        self._force_gather: bool = bool(getattr(self, "flex_kernel_options", {}).get("force_gather", False)) or (
            (os.environ.get("SGLANG_FLEX2_FORCE_GATHER") or "").strip() in _TRUE_ENVS
        )
        ...
        self._delegate_fa_impl: int = self._resolve_delegate_fa_impl()
        # Safety valve: many GPT-OSS configs use GQA (num_q_heads != num_kv_heads). Some
        # PyTorch FlexAttention codepaths (notably math fallback) will materialize K/V to
        # full query head count (repeat_interleave), causing multi-GB OOM at long context.
        # Default behavior is to auto-delegate to native FA3 when force_flash is requested
        # and GQA + large KV is detected.
        auto_delegate_env = (os.environ.get("SGLANG_FLEX2_AUTO_DELEGATE_ON_GQA") or "1").strip()
        self._auto_delegate_on_gqa: bool = auto_delegate_env in _TRUE_ENVS
        self._auto_delegate_kv_tokens_threshold: int = int(
            (os.environ.get("SGLANG_FLEX2_AUTO_DELEGATE_KV_TOKENS") or "8192").strip() or 8192
        )
        self._logged_auto_delegate: bool = False
        self._fa_delegate: object | None = None
        # If we are going to delegate, do it eagerly so forward-metadata / CUDA-graph
        # capture uses the delegate backend end-to-end (cannot switch mid-capture).
        if self._force_flash and self._auto_delegate_on_gqa and not self._delegate_to_fa:
            try:
                cfg = getattr(model_runner, "model_config", None)
                n_q = getattr(cfg, "num_attention_heads", None) if cfg is not None else None
                n_kv = (
                    getattr(cfg, "num_key_value_heads", None)
                    if cfg is not None
                    else getattr(cfg, "num_kv_heads", None) if cfg is not None else None
                )
                if isinstance(n_q, int) and isinstance(n_kv, int) and n_q != n_kv:
                    self._delegate_to_fa = True
                    self._ensure_fa_delegate(reason=f"init_force_flash_gqa_config_{n_q}v{n_kv}")
            except Exception:
                pass

        if self._delegate_to_fa:
            FlashAttentionBackend = _load_flashattention_backend()
            self._fa_delegate = FlashAttentionBackend(
                model_runner, fa_impl_ver=int(self._delegate_fa_impl)
            )
            if (os.environ.get("SGLANG_FLEX2_LOG_FORCE_FLASH") or "1").strip() in {"1", "true", "True"}:
                try:
                    print(
                        "[flex2] delegate_fa=1 delegate=FlashAttentionBackend(fa_impl_ver="
                        + str(int(self._delegate_fa_impl))
                        + ") force_flash="
                    ...
                    pass
        elif self._force_flash:
            # Blog-style "FlexFlash": request the CuTe/Flash path inside PyTorch FlexAttention.
            # Whether this is honored depends on the installed PyTorch build / device capability.
            if (os.environ.get("SGLANG_FLEX2_LOG_FORCE_FLASH") or "1").strip() in {"1", "true", "True"}:
                try:
                    print(
                        "[flex2] force_flash=1 delegate_fa=0 (requesting PyTorch FlexAttention CuTe/Flash backend)",
                        flush=True,
                    )
                except Exception:
                    pass
        if self._force_gather and (os.environ.get("SGLANG_FLEX2_LOG_FORCE_GATHER") or "1").strip() in {"1", "true", "True"}:
            try:
                print("[flex2] force_gather=1 (using gather-based FlexAttention path; paged global-KV disabled)", flush=True)
            except Exception:
                pass
        ...
        self._debug_shapes: bool = (os.environ.get("SGLANG_FLEX2_DEBUG_SHAPES") or "").strip() in _TRUE_ENVS
        ...
        self._logged_shapes_once: bool = False
    ...
    def _resolve_delegate_fa_impl(self) -> int:
        raw = getattr(self, "flex_kernel_options", {}).get("delegate_fa_impl", None)
        if raw is None:
            raw = (os.environ.get("SGLANG_FLEX2_DELEGATE_FA_IMPL") or "").strip()
        try:
            impl = int(raw) if raw not in (None, "") else 3
        except Exception:
            impl = 3
        if impl not in (3, 4):
            logger.warning(
                "Invalid FlexAttention2 delegate_fa_impl=%r; falling back to FA3.",
                raw,
            )
            impl = 3
        # Native SGLang FA4 is SM100-only. If a Flex* backend asks to delegate
        # to FA4 on Hopper/Ampere, clamp to FA3 so the control plane remains
        # truthful instead of creating an impossible backend combination.
        try:
            major, _minor = torch.cuda.get_device_capability()
        except Exception:
            major = None
        if impl == 4 and major is not None and major < 10:
            logger.warning(
                "FlexAttention2 delegate_fa_impl=4 requested on SM%s; clamping delegate to FA3.",
                major,
            )
            impl = 3
        return int(impl)

    def _ensure_fa_delegate(self, *, reason: str) -> None:
        if self._fa_delegate is None:
            FlashAttentionBackend = _load_flashattention_backend()
            self._fa_delegate = FlashAttentionBackend(
                self._model_runner, fa_impl_ver=int(self._delegate_fa_impl)
            )
        self._delegate_to_fa = True
        if not self._logged_auto_delegate and (os.environ.get("SGLANG_FLEX2_LOG_AUTO_DELEGATE") or "1").strip() in _TRUE_ENVS:
            self._logged_auto_delegate = True
            try:
                print(
                    f"[flex2] auto_delegate_fa_impl={int(self._delegate_fa_impl)} reason={reason}",
                    flush=True,
                )
            except Exception:
                pass
            logger.warning(
                "FlexAttention2 auto-delegated to FA%s (%s).",
                int(self._delegate_fa_impl),
                reason,
            )

    def _maybe_fastpath_fp8_kv_delegate(
        self,
        *,
        ...
        if k_cache.dtype in fp8_e5m2_dtypes or v_cache.dtype in fp8_e5m2_dtypes:
            raise RuntimeError(
                "FlexAttention2 does not support fp8_e5m2 KV-cache in this build (quality/perf). "
                "Use fp8_e4m3 or bf16 KV-cache."
            )

        is_fp8_kv = k_cache.dtype in fp8_e4m3_dtypes or v_cache.dtype in fp8_e4m3_dtypes

        if not is_fp8_kv:
            return False

        # High-performance path for force-flash Flex backends:
        # when KV is FP8, the old V1 gather fallback is correct but usually the wrong
        # tradeoff for long-context CUDA-graph serving. If we explicitly requested a
        # flash-style backend, route to the native FA delegate instead.
        if self._force_flash:
            self._ensure_fa_delegate(reason=reason)
            if not getattr(self, "_logged_fp8_kv_delegate", False):
                self._logged_fp8_kv_delegate = True
                logger.warning(
                    "FlexAttention2 KV-cache is float8; routing force_flash backend to FA%s delegate "
                    "instead of gather-based V1 fallback for graph stability/perf.",
                    int(self._delegate_fa_impl),
                )
            return True
    ...
    def _run_flex_forward_extend(
        ...
        if self._debug_shapes and not self._logged_shapes_once:
            self._logged_shapes_once = True
            try:
                bsz = int(seq_lens.shape[0])
                q_len = int(query.shape[0]) if hasattr(query, "shape") and len(query.shape) > 0 else -1
                kv_total_len = int(k_cache.shape[0])
                logger.info(
                    "Flex2 shapes(extend): bsz=%s q_len=%s kv_total_len=%s page_size=%s window_left=%s force_flash=%s",
                    bsz,
                    q_len,
                    kv_total_len,
                    int(self._page_size),
                ...
                pass
        # Safety guard: the Flex2 "global-KV" extend path can explode memory/time when the
        # global KV pool is very large (e.g. long-context GPT-OSS). In those regimes we
        # prefer the base (gather/slice) extend implementation, and reserve Flex2's
        # global-KV fastpath for decode (q_len=1) where it matters most.
        try:
            ps = int(self._page_size)
            kv_total_len = int(k_cache.shape[0])
            total_pages = int(kv_total_len // max(ps, 1)) if ps > 0 else 0
            max_pages_env = (os.environ.get("SGLANG_FLEX2_EXTEND_GLOBAL_KV_MAX_PAGES") or "").strip()
            max_pages = int(max_pages_env) if max_pages_env else 2048
            ...
            # Best-effort guard; if anything goes wrong, fall through to the existing logic.
            pass
        if (
        ...
        # FP8 KV-cache compatibility: force-flash fastpaths are handled at the top-level
        # forward entrypoint where we still have access to layer/forward_batch.
        try:
            fp8_e4m3_dtypes = tuple(
                dt
                for dt in (
    ...
    def forward_decode(
        ...
        **kwargs,
        ...
        # Auto-delegate to FA3 in the highest-risk configuration for FlexAttention OOM:
        # GQA + long KV + force_flash requested (often triggers PyTorch math fallback).
        try:
            use_gqa = layer.tp_q_head_num != layer.tp_k_head_num
        except Exception:
            use_gqa = False
        if self._force_flash and self._auto_delegate_on_gqa and use_gqa:
            kv_total_len = int(forward_batch.token_to_kv_pool.get_key_buffer(layer.layer_id).shape[0])
            if kv_total_len >= int(self._auto_delegate_kv_tokens_threshold):
                self._ensure_fa_delegate(reason=f"force_flash_gqa_kvlen{kv_total_len}")
                return self._fa_delegate.forward_decode(
                    q,
                    k,
                    v,
        ...
        # NOTE: do NOT route to V1 here. V1's gather path performs device->host copies
        # for per-request metadata and is not CUDA-graph capturable. If `force_gather`
        # is enabled, it is handled inside `_run_flex_forward_decode` (non-capturing only).
        if (os.environ.get("SGLANG_FLEX2_FORCE_FALLBACK") or "").strip() in {"1", "true", "True"}:
            return super().forward_decode(
                q,
                k,
        ...
        # - force_flash backend: route to native FA delegate for graph stability/perf
        # - otherwise: retain the V1 gather fallback for correctness
        if self._maybe_fastpath_fp8_kv_delegate(
            k_cache=k_cache,
            v_cache=v_cache,
            reason="decode_fp8_kv_force_flash",
        ):
            return self._fa_delegate.forward_decode(
                q,
                k,
        ...
        **kwargs,
        ...
        if self._force_flash and self._auto_delegate_on_gqa and use_gqa:
            try:
                ...
                kv_total_len = 0
            if kv_total_len >= int(self._auto_delegate_kv_tokens_threshold):
                self._ensure_fa_delegate(reason=f"extend_force_flash_gqa_kvlen{kv_total_len}")
                return self._fa_delegate.forward_extend(
                    q,
                    k,
                    v,
        ...
        try:
            ...
            k_cache = None
            v_cache = None
        if k_cache is not None and v_cache is not None and self._maybe_fastpath_fp8_kv_delegate(
            k_cache=k_cache,
            v_cache=v_cache,
            reason="extend_fp8_kv_force_flash",
        ):
            return self._fa_delegate.forward_extend(
                q,
                k,
        ...
        # NOTE: do NOT route to V1 here. See forward_decode note above.
    ...
    def _run_flex_forward_decode(
        ...
        if self._force_gather:
            # V1 gather path is not CUDA-graph capturable (it does device->host copies).
            # Only enable it when NOT capturing.
            try:
                if torch.cuda.is_current_stream_capturing():
                    pass
                else:
                ...
                # Best-effort; if the capture query fails, fall through to V2.
                pass
        if self._debug_shapes and not self._logged_shapes_once:
            self._logged_shapes_once = True
            try:
                ...
                kv_total_len = int(k_cache.shape[0])
                logger.info(
                    "Flex2 shapes(decode): bsz=%s q_len=%s kv_total_len=%s page_size=%s window_left=%s force_flash=%s",
                    bsz,
                    q_len,
                    kv_total_len,
                    int(self._page_size),
